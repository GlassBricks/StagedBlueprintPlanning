...
describe("setErrorHighlightAt", () => {
  ...
  test("deletes indicators only when all highlights removed", () => {
    ...
    for (let i = 2; i <= 5; i++) assert.not_nil(entity.getWorldEntity(i, "errorIndicator"), `layer ${i}`)
    highlightCreator.setErrorHighlightAt(assembly, entity, 1, false)
