...
import { UndoHandler, _simulateUndo } from "../../project/actions"
...
describe.each<[boolean, string]>([
  ...
  function assertCorrect(entity: LuaEntity, position: Position = pos): void {
    ...
    expect(project.actions.onEntityPossiblyUpdated).toHaveBeenCalledWith(entity, 1, expect._, 1, nil, nil)
  }
  ...
  test("tank has correct direction when not flipped ", () => {
    ...
    waitForPaste(() => {
      expect(project.actions.onEntityCreated).not.toHaveBeenCalled()
      expect(project.actions.onEntityPossiblyUpdated).toHaveBeenCalledWith(tank, 1, nil, player.index, nil, nil)
      if (useBplib) {
...
test("splitter has correct values when not flipped", () => {
  ...
  expect(project.actions.onEntityCreated).not.toHaveBeenCalled()
  expect(project.actions.onEntityPossiblyUpdated).toHaveBeenCalledWith(splitter, 1, nil, player.index, nil, nil)
})
