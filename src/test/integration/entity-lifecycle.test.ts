...
describe.each([
  ...
  test("is correct when entity dies", () => {
    const entity = buildEntity(3)
    const worldEntity = ctx.wp.getWorldEntity(entity, 4)!
  ...
  test("has error when entity cannot be placed at some stage", () => {
    ...
    const entity = buildEntity(3)
    expect(isPreviewEntity(ctx.wp.getWorldOrPreviewEntity(entity, 4)!)).toBe(true)
    ctx.assertEntityCorrect(entity, 4)
  })
  ...
  test("refreshWorldEntityAtStage will fix incorrect direction", () => {
    const entity = buildEntity(3)
    ctx.wp.getWorldOrPreviewEntity(entity, 4)!.direction = direction.north

    ctx.wp.refreshEntity(entity, 4)
    ctx.assertEntityCorrect(entity, false)
  })
  ...
  test("can create upgrade", () => {
    ...
    expect(ctx.wp.getWorldEntity(entity, 4)!.name).toBe(upgradeName)
    expect(ctx.wp.getWorldEntity(entity, 5)!.name).toBe(upgradeName)
    ctx.assertEntityCorrect(entity, false)
  })
  ...
  test("upgrading entities still updates error entity previews", () => {
    ...
    let preview = ctx.wp.getWorldOrPreviewEntity(entity, 5)!
    ...
    ctx.wp.getWorldOrPreviewEntity(entity, 4)!.order_upgrade({
      force: "player",
      target: upgradeName,
      player: ctx.player,
    })

    preview = ctx.wp.getWorldOrPreviewEntity(entity, 5)!
    expect(isPreviewEntity(preview)).toBe(true)
  ...
  if (diff) {
    ...
    test("refreshing and rebuilding an entity with diffs", () => {
      ...
      for (const stage of $range(1, 6)) {
        ctx.wp.refreshEntity(entity, stage)
        ctx.assertEntityCorrect(entity, false)
      }
      for (const stage of $range(1, 6)) {
        ctx.wp.rebuildEntity(entity, stage)
        ctx.assertEntityCorrect(entity, false)
      }
      for (const stage of $range(1, 6)) {
...
describe("multi-stage propagation", () => {
  ...
  test("entity at stage 1 has correct world entities at all stages", () => {
    ...
    for (const stage of $range(1, 6)) {
      const worldEntity = ctx.wp.getWorldEntity(entity, stage)
  ...
  test("modify at stage 3 propagates to stages 3-6 only", () => {
    ...
    const worldEntity3 = ctx.wp.getWorldEntity(entity, 3)!
    worldEntity3.inserter_stack_size_override = 2
    checkForEntityUpdates(worldEntity3, nil)

    for (const stage of $range(1, 2)) {
      const [savedValue] = saveEntity(ctx.wp.getWorldEntity(entity, stage)!)
  ...
  test("entity with lastStage has world entities only up to lastStage", () => {
    ...
    for (const stage of $range(1, 3)) {
      expect(ctx.wp.getWorldEntity(entity, stage)).toBeAny()
    }
    for (const stage of $range(4, 6)) {
      expect(ctx.wp.getWorldOrPreviewEntity(entity, stage)).toBeNil()
    }
    ctx.assertEntityCorrect(entity, false)
  })
})
...
describe("error state lifecycle", () => {
  ...
  test("error clears when blocker removed and entity rebuilt via cleanup tool", () => {
    ...
    expect(ctx.wp.hasErrorAt(entity, 4)).toBe(true)
    ctx.assertEntityCorrect(entity, 4)
    ...
    expect(ctx.wp.hasErrorAt(entity, 4)).toBe(false)
    ctx.assertEntityCorrect(entity, false)
  })
  ...
  test("error elsewhere indicator appears at non-error stages and clears when error fixed", () => {
    ...
    for (const stage of $range(2, 6)) {
      if (stage != 4) {
        expect(ctx.wp.entityStorage.get(entity, "errorElsewhereIndicator", stage))
