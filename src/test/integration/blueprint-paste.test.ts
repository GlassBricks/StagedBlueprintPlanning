...
describe("blueprinting", () => {
  test.each([true, false])("can maybe upgrade entity via blueprint, with super force build %s", (superForce) => {
    ...
    expect(ctx.wp.getWorldEntity(entity, 1)).toMatchTable({ name: expected })
  })
  ...
  test("can upgrade entity to different quality with super-force build", () => {
    ...
    expect(ctx.wp.getWorldEntity(entity, 1)).toMatchTable({
      name: "iron-chest",
  ...
  test("can upgrade entity to different quality in higher stage with super-force build", () => {
    ...
    expect(ctx.wp.getWorldEntity(entity, 2)).toMatchTable({
      name: "iron-chest",
      quality: { name: "legendary" },
    })
  })
  ...
  describe.each([defines.direction.north, defines.direction.northeast])("with rail direction %d", (diag) => {
    test.each([defines.direction.east, defines.direction.south])(
      ...
      (direction) => {
        ...
        expect(ctx.wp.getWorldEntity(projectEntity!, 1)).toEqual(rail)
  ...
  test("pasting diagonal rail at same position but different direction", () => {
    ...
    expect(ctx.wp.getWorldEntity(entity1!, 1)).toEqual(rail)
    expect(ctx.wp.getWorldEntity(entity2!, 1)).toEqual(rail2)
  })
  ...
  describe.each([
    ...
    test.each<[string, defines.direction, defines.direction, defines.build_mode]>([
      ...
      expect(ctx.wp.getWorldEntity(projEntity, 1)).toEqual(luaEntity)
  ...
  test("pasting two overlapping curved-rail-a matches both by position", () => {
    ...
    const rails = ctx.surfaces[0].find_entities_filtered({
      name: "curved-rail-a",
    ...
    const projEntity1 = ctx.project.content.findCompatibleWithLuaEntity(rail1, nil, 1)
    const projEntity2 = ctx.project.content.findCompatibleWithLuaEntity(rail2, nil, 1)
    expect(projEntity1).not.toBeNil()
    expect(projEntity2).not.toBeNil()
    expect(projEntity1).not.toEqual(projEntity2)

    expect(ctx.wp.getWorldEntity(projEntity1!, 1)).toEqual(rail1)
    expect(ctx.wp.getWorldEntity(projEntity2!, 1)).toEqual(rail2)
  })
  ...
  test("pasting rotate blueprint with a rotated fluid tank", () => {
    ...
    expect(ctx.wp.getWorldEntity(entity1, 1)).toEqual(tank)
  ...
  test("can paste a power pole at a lower stage to move", () => {
    ...
    expect(ctx.wp.getWorldEntity(pole, 2)).toEqual(pole2)
...
test("mirroring an entity", () => {
  ...
  const luaEntity = ctx.wp.getWorldEntity(chemPlant, 1)
  assert(luaEntity)
  ...
  const luaEntity2 = ctx.wp.getWorldEntity(chemPlant, 2)
...
test("mirroring an entity, via blueprint paste", () => {
  ...
  const worldEntity = ctx.wp.getWorldEntity(chemPlant, 1)!
  ...
  expect(ctx.wp.getWorldEntity(chemPlant, 1)?.mirroring).toBe(false)
  expect(ctx.wp.getWorldEntity(chemPlant, 2)?.mirroring).toBe(false)
})

// 2.1 added mirroring to inserters and mining drills. The mod's mirror handling is generic
// (save via create_blueprint, apply via LuaEntity.mirroring), so these should round-trip with
// no type-specific code.
test.each(["inserter", "burner-mining-drill"])("mirroring a %s", (name) => {
  const entity = ctx.buildEntity(1, { name })
  const luaEntity = ctx.wp.getWorldEntity(entity, 1)
  assert(luaEntity)
  luaEntity.mirroring = true
  Events.raiseFakeEventNamed("on_player_flipped_entity", {
    entity: luaEntity,
  ...
  expect(ctx.wp.getWorldEntity(entity, 2)?.mirroring).toBe(true)
